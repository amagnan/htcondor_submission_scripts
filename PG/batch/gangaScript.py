...
config['Output']['MassStorageFile']['uploadOptions']['path'] = '/eos/experiment/ship/user/ammagnan/PG260421/'
config['Output']['MassStorageFile']['uploadOptions']['defaultProtocol'] = 'root://eospublic.cern.ch//eos/experiment/ship/user/ammagnan/PG260421/'
...
evtsPerJob = 5000  # 200000
nJ = 1  # 
nSJ = 1  # fixed number of subjobs per job to register all subjobs on rucio at the same time

startRun = 0 #random.randint(run_min, run_max - nJ * nSJ)

for J in range(nJ):
    j = Job(name = f'run sim production number {J} - {nSJ * evtsPerJob} events')
    j.application = Executable(exe = File('wn_script.py'), args = ['--fs-install',FS_INSTALL,'--runfile', 'run_simScript.py', '--cvmfs_version', '26.03', '--site', SITE, '--', '-o', '"./"', '-n', evtsPerJob,'PG --multiplePG --Estart 1 --Eend 400 --pID 13 --Dx 440 --Dy 640 --Vz 3200 --'])
    
    # IMPORTANT: Only put the run seed in the splitter arguments
    j.splitter = ArgSplitter(args = [['--seed', startRun + J * nSJ + _i] for _i in range(nSJ)], append = True)
