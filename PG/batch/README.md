...
``` source build.sh```
to create the test_config.sh which contains all the environment corresponding to the FairShip version in your local sw/ and that will be used for running on the batch system, if using bashScript.sh
...
2. Make sure the FairShip installation in `bashScript.sh` points to the version you want -  if using bashScript.sh
...
Also good practice to execute the command interactively first, with all necessary argument as given in gangaScript.py, to test everything is in order before submitting.
